...
class DDPM(nn.Module):
    def __init__(
        ...
        *,
        ...
        self.register_buffer('sqrt_recip_gammas', torch.from_numpy(np.sqrt(1. / gammas)).float())
        self.register_buffer('sqrt_recipm1_gammas', torch.from_numpy(np.sqrt(1. / gammas - 1)).float())
    ...
    def forward(self, y_0, y_cond=None, noise=None):
        ...
        noise_hat = self.denoise_fn(torch.cat([y_cond, y_noisy], dim=1), sample_gammas.squeeze())
...
if __name__ == "__main__":
    bs = 8
    diffusion = DDPM().cuda()
    inputL = torch.randn([bs, 1, 224, 224]).cuda()
    inputAB = torch.randn([bs, 2, 224, 224]).cuda()
    loss = diffusion(y_0 = inputAB, y_cond = inputL)